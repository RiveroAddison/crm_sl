...
export async function getDashboard(req: Request, res: Response) {
  try {
    ...
    const clients = await prisma.clienteEmpresa.findMany({ where: { empresaId: context.tenantId, ...(context.rol === 'VENDEDOR' ? { vendedor: context.user.nombre } : {}) }, include: { clienteCorporativo: { include: { crossSellingMatriz: true } }, visitas: true, ventas: { orderBy: { fecha: 'asc' } } }, orderBy: { updatedAt: 'desc' } });
