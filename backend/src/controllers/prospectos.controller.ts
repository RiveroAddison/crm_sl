...
export async function list(req: Request, res: Response) {
  try { const context = req.context as RequestContext; const items = await prisma.oportunidad.findMany({ where: { empresaId: context.tenantId, ...(context.rol === 'VENDEDOR' ? { vendedorId: context.userId } : {}) }, include: { actividades: { include: { autor: { select: { id: true, nombre: true } } }, orderBy: { fecha: 'desc' } } }, orderBy: { createdAt: 'desc' } }); return res.json({ success: true, data: items.map(response), error: '' }); }
  catch (err) { logger.error({ err }, '[prospectos] Error'); return res.status(500).json({ success: false, data: null, error: 'No fue posible cargar los prospectos' }); }
}
...
export async function updateStage(req: Request, res: Response) {
  ...
  } catch (cause) { logger.error({ err: cause }, 'Error al cambiar etapa del prospecto'); const message = sanitizeError(cause, 'No fue posible actualizar el prospecto'); return res.status(errorStatus(message)).json({ success: false, data: null, error: message }); }
}

export async function remove(req: Request, res: Response) {
  try { const prospectId = String(req.params.id); const context = req.context as RequestContext; const item = await prisma.oportunidad.findFirst({ where: { id: prospectId, empresaId: context.tenantId, ...(context.rol === 'VENDEDOR' ? { vendedorId: context.userId } : {}) } }); if (!item) return res.status(404).json({ success: false, data: null, error: 'Prospecto no encontrado' }); await prisma.oportunidad.delete({ where: { id: item.id } }); return res.json({ success: true, data: { id: item.id }, error: '' }); }
  catch (err) { logger.error({ err }, '[prospectos] Error'); return res.status(500).json({ success: false, data: null, error: 'No fue posible eliminar el prospecto' }); }
}
