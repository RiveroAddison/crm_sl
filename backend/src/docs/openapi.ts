...
export const openapi = {
  ...
  paths: {
    ...
    '/api/profit/sync/clientes': { post: { tags: ['Profit'], summary: 'Sincroniza clientes desde Profit', operationId: 'syncClients', security: [{ sessionCookie: [] }], requestBody: jsonBody({ $ref: '#/components/schemas/SyncRequest' }), responses: { '200': response(), '404': errorResponse('No hay empresas activas'), ...adminResponses } } },
    '/api/profit/sync/ventas': { post: { tags: ['Profit'], summary: 'Sincroniza ventas desde Profit', operationId: 'syncSales', security: [{ sessionCookie: [] }], requestBody: jsonBody({ $ref: '#/components/schemas/SyncRequest' }), responses: { '200': response(), '404': errorResponse('No hay empresas activas'), ...adminResponses } } },
    '/api/profit/sync/all': { post: { tags: ['Profit'], summary: 'Sincroniza clientes y ventas', operationId: 'syncAll', security: [{ sessionCookie: [] }], requestBody: jsonBody({ $ref: '#/components/schemas/SyncRequest' }), responses: { '200': response(), ...adminResponses } } },
  ...
  components: {
    ...
    schemas: {
      ...
      LeadRequest: { type: 'object', required: ['nombreContacto', 'empresaNombre', 'fuente'], properties: { nombreContacto: { type: 'string', minLength: 2 }, empresaNombre: { type: 'string', minLength: 2 }, rif: { type: 'string', pattern: '^[JVEGjveg]-[0-9]{8,9}-[0-9]$' }, email: { type: 'string', format: 'email' }, telefono: { type: 'string' }, fuente: { type: 'string', enum: ['REDES', 'WEB', 'LLAMADA', 'REFERIDO'] }, estadoCalificacion: { type: 'string', enum: ['NUEVO', 'CALIFICADO', 'DESCARTADO'] }, presupuesto: { type: 'number', minimum: 0 }, necesidad: { type: 'string' }, autoridad: { type: 'string' }, tiempo: { type: 'string' }, vendedorId: { type: 'string', format: 'uuid' } } },
      LeadPatchRequest: { type: 'object', description: 'Todos los campos son opcionales.', properties: { nombreContacto: { type: 'string', minLength: 2 }, empresaNombre: { type: 'string', minLength: 2 }, rif: { type: 'string', pattern: '^[JVEGjveg]-[0-9]{8,9}-[0-9]$' }, email: { type: 'string', format: 'email' }, telefono: { type: 'string' }, fuente: { type: 'string', enum: ['REDES', 'WEB', 'LLAMADA', 'REFERIDO'] }, estadoCalificacion: { type: 'string', enum: ['NUEVO', 'CALIFICADO', 'DESCARTADO'] }, presupuesto: { type: 'number', minimum: 0 }, necesidad: { type: 'string' }, autoridad: { type: 'string' }, tiempo: { type: 'string' }, vendedorId: { type: 'string', format: 'uuid' } } },
      OrderRequest: { type: 'object', required: ['detalles'], properties: { clienteEmpresaId: { type: 'string', format: 'uuid' }, oportunidadId: { type: 'string', format: 'uuid' }, detalles: { type: 'array', minItems: 1, items: { type: 'object', required: ['producto', 'cantidad', 'precioUnitario'], properties: { producto: { type: 'string', minLength: 1 }, cantidad: { type: 'integer', exclusiveMinimum: 0 }, precioUnitario: { type: 'number', minimum: 0 } } } } }, description: 'Debe indicar clienteEmpresaId u oportunidadId.' },
