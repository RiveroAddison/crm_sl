...
// Formato venezolano: separador de miles con punto, decimales con coma.
// Ej: 3500 -> "3.500", 12500.5 -> "12.500,5".
const litersFormatter = new Intl.NumberFormat('es-VE', {
...
<template>
  <section class="bg-white p-6 rounded-xl shadow-sm border border-slate-100 mb-8">
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
      ...
      <div class="flex items-center gap-4">
        <div class="flex items-center gap-2 text-xs text-slate-500">
          ...
          <span class="w-3 h-3 bg-brand-green rounded inline-block ml-2"></span> Litros
        </div>
      </div>
    </div>
    ...
    <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 mb-6 overflow-x-auto">
      ...
      <div class="flex items-end gap-3 h-48 pt-4 px-2 min-w-[500px]">
        <button
          ...
          :aria-label="`Mes ${month}: $${Math.round(sales[index]).toLocaleString('es-VE')} y ${(units[index] ?? 0).toLocaleString('es-VE')} litros`"
          @click="$emit('update:selectedMonth', month)"
        >
          <!-- Badge con el total de litros vendidos en el mes (oculto si es 0) -->
          <span
            v-if="(units[index] ?? 0) > 0"
            class="text-[10px] sm:text-xs font-bold text-white bg-emerald-600 rounded-full px-2 py-0.5 mb-1 shadow-sm whitespace-nowrap"
            :title="`${litersFormatter.format(units[index] ?? 0)} litros vendidos`"
          >
            {{ litersFormatter.format(units[index] ?? 0) }} L
          </span>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 bg-brand-sky p-4 rounded-lg border border-sky-100">
      ...
      <strong class="text-brand-blue text-sm">Total Mes: <span class="text-brand-green-dark font-bold">${{ selectedMonthTotal.toLocaleString('es-VE') }}</span> (<span class="text-slate-700">{{ litersFormatter.format(selectedMonthUnits) }} L</span>)</strong>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <article v-for="(amount, index) in weeklyBreakdown" :key="index" class="p-4 rounded-lg bg-slate-50 border border-slate-100 flex flex-col justify-between">
        ...
        <small class="text-xs text-slate-400 mt-1">{{ litersFormatter.format(selectedMonthSales.filter((s) => s.semana === index + 1).reduce((total, s) => total + s.unidades, 0)) }} litros</small>
      </article>
    </div>
  </section>
</template>
